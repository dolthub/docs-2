// Crawls every page in the DoltLab docs and verifies that all internal links
// resolve to a real page (HTTP 200).
export {};

import { coreDoltlabPages } from "../../fixtures/doltlab-pages";
...
// Use coreDoltlabPages (excludes release notes) to keep test duration reasonable
const batchSize = 10;
const batches: Array<{ name: string; pages: typeof coreDoltlabPages }> = [];
for (let i = 0; i < coreDoltlabPages.length; i += batchSize) {
  const batch = coreDoltlabPages.slice(i, i + batchSize);
  const first = batch[0].path;
  const last = batch[batch.length - 1].path;
  batches.push({ name: `${first} ... ${last}`, pages: batch });
}

describe("DoltLab docs — dead link check", () => {
