...
`EMAIL_PORT`, port to the existing SMTP server. By default, this is assumed to be a `STARTTLS` port. To use an implicit TLS port, [please follow these steps](/older/pre-installer-administrator-guide#connect-to-an-smtp-server-with-implicit-tls).<br/>
...
If you are experiencing any SMTP server connection issues (or DoltLab account creation issues) please see [the SMTP troubleshooting guide](/older/pre-installer-administrator-guide#troubleshoot-smtp-server-connection-problems).
