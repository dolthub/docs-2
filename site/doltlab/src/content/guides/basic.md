...
To authenticate a Dolt client against a DoltLab remote, use the `--auth-endpoint`, `--login-url`, and `--insecure` arguments with the [dolt login](https://dolthub.com/docs/cli-reference/cli#dolt-login) command.
...
To authenticate without using the `dolt login` command, first run the [dolt creds new](https://dolthub.com/docs/cli-reference/cli#dolt-creds-new) command, which will output a new public key:
...
In DoltLab >= v2.3.12, the unused `doltlabdb-dolt-configs` volume has been removed, in favor of explicitly generating and mounting a [config.yaml](https://dolthub.com/docs/sql-reference/server/configuration#config.yaml) file used to configure the `doltlabdb` application server.
