...
Use `start.sh` to start your DoltLab instance and `stop.sh`, to stop it. These scripts will reference the other local files generated by the [installer](/reference/installer) as well.

DoltLab's [installer](/reference/installer) uses a configuration file, `./installer_config.yaml`, that ships with DoltLab. The default configuration contains the following:
...
## see /guides/administrator#connect-smtp-server.
#smtp:
#  auth_method: "plain"
#  host: ""
#  port: 0
...
These passwords can be changed later on, but requires a [couple additional steps](/guides/basic#update-application-database-passwords).
...
Notice the output produced by the [installer](/reference/installer) as shown below:
...
For more ways to configure and run your DoltLab instance, checkout [DoltLab's Administrator Guide](/guides/basic).
