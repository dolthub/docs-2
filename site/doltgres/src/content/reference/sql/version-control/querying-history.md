...
set](/concepts/git/working-set) that have not been committed to that head via `select
...
branches](/reference/version-control/branches) for more details.

Note that this syntax applied to a branch will name that branch's [working
set](/concepts/git/working-set) and therefore includes any changes not yet committed to
...
branch](/reference/version-control/branches), e.g.:
