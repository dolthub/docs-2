...
[the `dolt_reflog()` table function](/reference/version-control/dolt-sql-functions#dolt_reflog) to see the history of commits
your branch has referenced and either recreate the branch from the last referenced commit with
[the `dolt_branch()` function](/reference/version-control/dolt-sql-functions#dolt_branch) or reset the branch to a
previous commit with [the `dolt_reset()` function](/reference/version-control/dolt-sql-functions#dolt_reset). See
[the `dolt_reflog()` table function](/reference/version-control/dolt-sql-functions#dolt_reflog) for an example of recreating
