...
supports a [logical replication mode](/guides/replication-from-postgres), where you configure a
...
Replication](/reference/server/replication#replication-through-a-remote). In this mode
the primary and the read replicas are completely decoupled. The primary and the read replicas
leverage a shared, Git-style [remote](/concepts/git/remotes) to facilitate replication. On the primary,
