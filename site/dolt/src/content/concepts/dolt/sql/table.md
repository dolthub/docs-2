...
Tables are the core unit of database [schema](./schema). Tables are defined by a set of columns. Columns can be [primary keys](./primary-key) which act as a unique identifier for each row. Once a table schema is defined, rows containing data can be inserted into the table.
...
Structure the data in your database into tables. Define relationships between tables using foreign key [constraints](./constraints). Use `CREATE` statements to create tables and `ALTER` statements to change their schema.
