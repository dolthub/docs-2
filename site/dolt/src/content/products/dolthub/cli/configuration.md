...
dh config set repo OWNER/people
dh db view
```
...
2. `DH_REPO` in the environment.
3. The saved `repo` setting.
...
| `DH_REPO` | Default database, in `[HOST/]OWNER/DB` form. |
...
```bash
DH_REPO=OWNER/people dh sql --ref main "SELECT COUNT(*) FROM people"
```

To return to saved defaults, unset the environment overrides:

```bash
unset DH_HOST DH_REPO
```
...
`config list` shows the effective `host` and `repo` values and their sources. It does not discover local remotes. The supported writable keys are `host` and `repo`; there is no `config unset` command. You can select another saved value or use an explicit selector for one invocation.
