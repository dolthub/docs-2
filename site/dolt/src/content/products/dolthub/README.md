...
DoltHub has many uses. We recommend getting started by [sharing a database](./data-sharing).
...
DoltHub has [an API](./api/) you can script against. The documentation covers:

1. [Authentication](./api/authentication)
2. [SQL API](./api/sql) - Used to make read or write SQL queries to a DoltHub database
3. [CSV API](./api/csv) - Used to download CSV format files of DoltHub tables
4. [Database API](./api/database) - Used to interact with DoltHub databases and pull requests
5. [Hooks](./api/hooks) - Used to receive change events to your DoltHub databases

# Guides

- [Transform File Uploads](./transform-uploads)

We added a feature to [Transform DoltHub File Uploads](./transform-uploads). This allows you to set up an API to receive and transform files that are uploaded to your databases on DoltHub. The transformed files are sent back to DoltHub for import to a DoltHub database.
