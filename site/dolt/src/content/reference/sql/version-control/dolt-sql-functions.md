...
does not include unversioned items such as tables which have been [ignored](dolt-system-tables#dolt_ignore). The function
...
`DOLT_DIFF_STAT()` works like [CLI `dolt diff --stat` command](../../cli/cli#dolt-diff), but two commits are required to use the `DOLT_DIFF_STAT()` table function and the table name is optional. For keyless tables, this table function only provides the number of added and deleted rows. It returns empty result for tables with no data changes.
...
`DOLT_DIFF_SUMMARY()` works like [CLI `dolt diff --summary` command](../../cli/cli#dolt-diff),
...
works like [CLI `dolt log` command](../../cli/cli#dolt-log).
...
equivalent of [`dolt diff -r sql` CLI command](../../cli/cli#dolt-diff).
...
The `DOLT_TEST_RUN()` table function executes tests defined in the [`dolt_tests` system table](dolt-system-tables#dolt_tests) and returns the results. Tests can be run individually by name, by test group, or all at once.
