...
procedure](dolt-sql-procedures#doltmerge):
...
merge using the [dolt_merge_status](./dolt-system-tables#doltmergestatus) system table:
...
[common-ancestor](../../cli/cli#dolt-merge-base) and describes why the `ours` and `theirs`
...
or resolve the schema conflict using [`dolt_conflicts_resolve()`](./dolt-sql-procedures#doltconflictsresolve).
...
See [`dolt_conflicts_$tablename`](./dolt-system-tables#doltconflictstablename) for details.
...
[`dolt_commit()` system function](./dolt-sql-procedures#doltcommit)
or with the [`@@dolt_transaction_commit` system
variable](./dolt-sysvars#dolttransactioncommit)) if the working
