...
If you determine you need to disable Automatic GC in the `sql-server` context, you must set the following [configuration](./configuration.md):
...
If you have disabled Automatic GC, you can run garbage collection on your running SQL server using [`call dolt_gc`](../version-control/dolt-sql-procedures.md#dolt_gc) through any connected client. To prevent concurrent
