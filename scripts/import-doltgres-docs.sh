#!/bin/bash
# Imports all markdown files from the doltgres docs into the doltgres site.

DOCS_CONTENT="/Users/taylor/go/src/github.com/dolthub/docs/packages/doltgres/content"
SITE_CONTENT="/Users/taylor/go/src/github.com/dolthub/docs-2/site/doltgres/src/content"
SITE_PAGES="/Users/taylor/go/src/github.com/dolthub/docs-2/site/doltgres/src/pages"
...
echo "Copied $(find "$SITE_CONTENT" -name "*.md" | wc -l | tr -d ' ') doltgres markdown files"
...
def resolve_link(m):
    ...
    # Doltgres URL remapping: /reference/sql/* → /reference/* (drop /sql/)
    import re as re2
    resolved = re2.sub(r'^/reference/sql/', '/reference/', resolved)
...
find "$SITE_CONTENT" -name "*.md" -not -path "*/.gitbook/*" | sort | while read -r mdfile; do
  ...
  # Doltgres URL remapping: reference/sql/* → reference/* (drop sql/)
  pagepath=$(echo "$pagepath" | sed 's|^reference/sql/|reference/|')
